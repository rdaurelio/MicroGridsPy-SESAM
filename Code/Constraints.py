...
class Constraints_Greenfield_Milp():
    ...
    def Minimun_Charge(model,s,yt,ut,t): # Minimun state of charge
        return model.Battery_SOC[s,yt,t] >= model.Battery_Units[ut]*model.Battery_Nominal_Capacity_milp*model.Battery_Depth_of_Discharge
    ...
    if Generator_Partial_Load:
     ...
     def Maximum_Generator_Energy_Total(model,s,yt,ut,g,t):
     ...
     def Maximun_Generator_Energy_Total(model,s,yt,ut,g,t): 
        return model.Generator_Energy_Total[s,yt,g,t] <= model.Generator_Units[ut,g]*model.Generator_Nominal_Capacity_milp[g]*model.Delta_Time
...
class Constraints_Brownfield_Milp():
    ...
    def Investment_Cost(model):  
        ...
        Inv_Ren = sum(((model.RES_Units_milp[1,r]-model.RES_units[r])*model.RES_Nominal_Capacity[r]*model.RES_Specific_Investment_Cost[r])
                        + sum((((model.RES_Units_milp[ut,r] - model.RES_Units_milp[ut-1,r])*model.RES_Nominal_Capacity[r]*model.RES_Specific_Investment_Cost[r]))/((1+model.Discount_Rate)**(yt-1))
                        for (yt,ut) in tup_list) for r in model.renewable_sources)  
    ...
    def Salvage_Value(model):   
        ...
        if model.Steps_Number == 1:    
            SV_Ren_1 = sum(((model.RES_Units_milp[1,r]-model.RES_units[r])*model.RES_Nominal_Capacity[r])*model.RES_Specific_Investment_Cost[r] * (model.RES_Lifetime[r]-model.Years)/model.RES_Lifetime[r] / 
                            ((1 + model.Discount_Rate)**(model.Years)) for r in model.renewable_sources)+sum((model.RES_units[r]*model.RES_Nominal_Capacity[r])*model.RES_Specific_Investment_Cost[r] * (model.RES_Lifetime[r]-model.RES_years[r]-model.Years)/model.RES_Lifetime[r] / 
                            ((1 + model.Discount_Rate)**(model.Years)) for r in model.renewable_sources)        
            SV_Ren_2 = 0 
            SV_Ren_3 = 0    
    ...
    def RES_emission(model): #LCA emissions of RES
        ...
        return model.RES_emission == sum(model.RES_unit_CO2_emission[r]*(model.RES_Units_milp[1,r]-model.RES_units[r])*model.RES_Nominal_Capacity[r]/1e3 for r in model.renewable_sources)+sum(sum(model.RES_unit_CO2_emission[r]*(model.RES_Units_milp[ut,r]-model.RES_Units_milp[ut-1,r])*model.RES_Nominal_Capacity[r]/1e3 for (yt,ut) in tup_list) for r in model.renewable_sources)
